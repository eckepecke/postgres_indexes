...
# def main():
#     parser = argparse.ArgumentParser(description='Batch process benchmark results for outlier detection')
#     parser.add_argument('--input_dir', default='../TPCC_results', 
#                         help='Directory containing CSV files to analyze (default: ../TPCC_results)')
#     parser.add_argument('--metrics', nargs='+', default=['NOPM', 'TPM'],
#                         help='Metrics to analyze for outliers (default: NOPM TPM)')
...
def detect_outliers(data, column, method='iqr'):
    ...
    if method == 'iqr':
        ...
        data[f'is_outlier_{column}'] = (data[column] < lower_bound) | (data[column] > upper_bound)
...
def process_file(file_path, metrics, output_dir=None, exclude_outliers=False):
    ...
    try:
        ...
        # Initialize a combined DataFrame for all metrics
        ...
        for metric in metrics:
            ...
            # Get outlier information
            outlier_col = f'is_outlier_{metric}'
            outlier_count = data_with_outliers[outlier_col].sum()
            total_count = len(data_with_outliers)
            print(f"Found {outlier_count} outliers out of {total_count} data points ({outlier_count/total_count*100:.2f}%)")
            
            if outlier_count > 0:
                print("\nOutliers:")
                print(data_with_outliers[data_with_outliers[outlier_col]])
            
            # Print summary statistics for non-outliers
            non_outliers = data_with_outliers[~data_with_outliers[outlier_col]]
            if not non_outliers.empty:
                print("\nSummary statistics for non-outliers:")
                print(f"Count: {len(non_outliers)}")
                print(f"Median {metric}: {non_outliers[metric].median()}")
            ...
            # Merge the outlier information into the combined DataFrame
            combined_data = pd.merge(combined_data, 
                                   data_with_outliers[[metric, outlier_col]], 
                                   on=metric, 
                                   how='left')
        ...
        if output_dir:
            ...
            base_name = os.path.splitext(file_name)[0]
            output_path = os.path.join(output_dir, f"{base_name}_analysis.csv")
            
            if exclude_outliers:
                # Filter out rows that are outliers in any metric
                outlier_cols = [f'is_outlier_{m}' for m in metrics if f'is_outlier_{m}' in combined_data.columns]
                if outlier_cols:
                    non_outliers_mask = combined_data[outlier_cols].sum(axis=1) == 0
                    combined_data[non_outliers_mask].drop(columns=outlier_cols).to_csv(output_path, index=False)
                else:
                    combined_data.to_csv(output_path, index=False)
                print(f"\nNon-outlier data saved to {output_path}")
            else:
                # Save all data with outlier information
                combined_data.to_csv(output_path, index=False)
                print(f"\nData with outlier information saved to {output_path}")
